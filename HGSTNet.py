...
from TransformerEncode import TransformerEncoder
...
class GCNLayer_PyG(nn.Module):
    ...
    def forward(self, H, prior_A=None):
        # print(f"GCNLayer_PyG 输入特征形状: {H.shape}")  # 打印输入特征形状
        H = self.BN(H)
        ...
        # print(f"GCNLayer_PyG 输出特征形状: {out.shape}")  # 打印输出特征形状
        return out, A
...
class HiGCN(nn.Module):
    def __init__(self, height: int, width: int, changel: int, class_count: int,
        ...
        # self.tfe1 = TransformerEncoder(in_ch=128)
        # self.tfe2 = TransformerEncoder(in_ch=64)
        # self.tfe3 = TransformerEncoder(in_ch=32)
        # self.tfe4 = TransformerEncoder(in_ch=16)
    ...
    def forward(self, x: torch.Tensor, showFlag=False):
        ...
        ###########
        x = encoder_features[0].permute([1, 0])
        x = x.reshape([128, 145, 145])
        H_0 = torch.unsqueeze(x, 0)
        tfe1_out=self.tfe1(H_0)
        encoder_features[0] = torch.squeeze(tfe1_out, 0).permute([1, 2, 0]).reshape([h * w, -1])
        ######################
        x = encoder_features[1].permute([1, 0])
        x = x.reshape([64, 64, 32])
        H_0 = torch.unsqueeze(x, 0)
        tfe2_out = self.tfe2(H_0)
        encoder_features[1] =torch.squeeze(tfe2_out, 0).permute([1, 2, 0]).reshape([64 * 32, -1])
        #################################
        x = encoder_features[2].permute([1, 0])
        x = x.reshape([32, 32, 32])
        H_0 = torch.unsqueeze(x, 0)
        tfe3_out=self.tfe3(H_0)
        encoder_features[2] = torch.squeeze(tfe3_out, 0).permute([1, 2, 0]).reshape([32 * 32, -1])
        ############################################
        x = encoder_features[3].permute([1, 0])
        x = x.reshape([16, 16, 32])
        H_0 = torch.unsqueeze(x, 0)
        tfe4_out=self.tfe4(H_0)
        encoder_features[3] = torch.squeeze(tfe4_out, 0).permute([1, 2, 0]).reshape([16 * 32, -1])
        ...
        if len(self.S_list) > 0:
            H_i = torch.mm(self.S_list[0],H_i) # hw*d
        H_i = torch.cat([H_i, encoder_features[0]], dim=-1)
        ...
        # return Y, A_encoders, A_decoders
